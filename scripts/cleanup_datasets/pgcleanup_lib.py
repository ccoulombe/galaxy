...
def available_actions():
    ...
    for name, value in inspect.getmembers(sys.modules[__name__]):
        if not name.startswith("_") and inspect.isclass(value) and value != Action and issubclass(value, Action):
            actions[value.name_c()] = value
    return actions


def build_cleanup_parser(actions):
    parser = argparse.ArgumentParser()
    populate_config_args(parser)
    parser.add_argument("-d", "--debug", action="store_true", default=False, help="Enable debug logging (SQL queries)")
    ...
    parser.add_argument("-s", "--sequence", dest="sequence", default="", help="DEPRECATED: Comma-separated sequence of actions")
    parser.add_argument("-w", "--work-mem", dest="work_mem", default=None, help="Set PostgreSQL work_mem for this connection")
    parser.add_argument(
        "-b",
        "--batch-size",
        dest="batch_size",
    ...
    parser.add_argument(
        ...
        metavar="ACTION",
        default=[],
        help="Action(s) to perform, chosen from: {}".format(", ".join(sorted(actions.keys()))),
    )
    return parser


def parse_cleanup_args(parser, argv=None):
    args = parser.parse_args(argv)
    args.sequence = [x.strip() for x in args.sequence.split(",")]
    if args.sequence != [""]:
        args.actions.extend(args.sequence)
    if not args.actions:
        parser.error("Please specify one or more actions")
    return args


def _execute_select(conn, sql, args):
    stmt = text(sql)
    args = args.copy()
...
def _append_notification(notification_map, seen, email, dataset_name, history_name):
    if not email:
        return
    entry = (dataset_name, history_name)
    if entry not in seen[email]:
        notification_map[email].append(entry)
        seen[email].add(entry)


def notifications_for_hda_ids(conn, hda_ids):
    if not hda_ids:
        return {}
    sql = """
        SELECT galaxy_user.email AS email,
               history_dataset_association.name AS dataset_name,
               history.name AS history_name
          FROM history_dataset_association
          JOIN history ON history.id = history_dataset_association.history_id
          JOIN galaxy_user ON galaxy_user.id = history.user_id
         WHERE history_dataset_association.id IN :hda_ids
      ORDER BY galaxy_user.email, history.name, history_dataset_association.name
    """
    notifications = defaultdict(list)
    seen = defaultdict(set)
    for email, dataset_name, history_name in _execute_select(conn, sql, {"hda_ids": sorted(hda_ids)}):
        _append_notification(notifications, seen, email, dataset_name, history_name)
    return dict(notifications)


def notifications_for_dataset_ids(conn, dataset_ids):
    if not dataset_ids:
        return {}
    sql = """
        SELECT galaxy_user.email AS email,
          ...
          JOIN history ON history.id = history_dataset_association.history_id
          JOIN galaxy_user ON galaxy_user.id = history.user_id
         WHERE history_dataset_association.dataset_id IN :dataset_ids
      ORDER BY galaxy_user.email, history.name, history_dataset_association.name
    """
    notifications = defaultdict(list)
    seen = defaultdict(set)
    for email, dataset_name, history_name in _execute_select(conn, sql, {"dataset_ids": sorted(dataset_ids)}):
        _append_notification(notifications, seen, email, dataset_name, history_name)
    return dict(notifications)


def merge_notifications(target, source):
    seen = defaultdict(set, {email: set(datasets) for email, datasets in target.items()})
    for email, datasets in source.items():
        for dataset_name, history_name in datasets:
            _append_notification(target, seen, email, dataset_name, history_name)
    return target
...
class Action:
    ...
    primary_key = None
    notification_target_column = None
    ...
    def __init__(self, app):
        ...
        self.__post_methods = []
        self.__exit_methods = []
        self._notification_targets = set()
    ...
    def _init(self):
        ...
        """
        if self.notification_target_column and self.notification_target_type:
            self._register_row_method(self.collect_notification_target)

    def collect_notification_target(self, row):
        target = getattr(row, self.notification_target_column, None)
        if target:
            self._notification_targets.add(target)

    def collect_notifications(self):
        if self.notification_target_type == "dataset":
            return notifications_for_dataset_ids(self._conn, self._notification_targets)
        if self.notification_target_type == "hda":
            return notifications_for_hda_ids(self._conn, self._notification_targets)
        return {}
...
class UpdateHDAPurgedFlag(Action):
    ...
    notification_target_column = "purged_hda_id"
    notification_target_type = "hda"
    _action_sql = """
        WITH batch_ids
...
class DeleteExportedHistories(Action):
    ...
    notification_target_column = "deleted_dataset_id"
    notification_target_type = "dataset"
    _action_sql = """
...
class PurgeDatasets(RemovesDatasets, Action):
    ...
    notification_target_column = "purged_dataset_id"
    notification_target_type = "dataset"
    _action_sql = """
...
class Cleanup:
    def __init__(self, args=None, argv=None, preview=False):
        self.__actions = available_actions()
        self.args = args or parse_cleanup_args(build_cleanup_parser(self.actions), argv=argv)
        self.config = None
        self.__conn = None
    ...
    def _dry_run_event(self):
        ...
        if max_id is None:
            ...
            if self.preview:
                log.info("Created a temporary dry-run event for preview mode")
            else:
                self.conn.commit()
    ...
    def _run_action(self, action):
        ...
        return action.collect_notifications()
    ...
    def _run_actions(self):
        ...
        for name in self.args.actions:
            ...
            with cls(self) as action:
                merge_notifications(notifications, self._run_action(action))
    ...
    def preview_notifications(self):
        original_dry_run = self.args.dry_run
        self.args.dry_run = True
        try:
            return self._run_actions()
        finally:
            self.args.dry_run = original_dry_run


def main(argv=None):
    with Cleanup(argv=argv) as app:
        try:
            app.run()
        except Exception:
